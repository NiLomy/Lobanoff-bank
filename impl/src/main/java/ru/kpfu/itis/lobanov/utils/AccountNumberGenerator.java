...
public class AccountNumberGenerator {
    ...
    public String generatePersonalAccountNumber() {
        String organizationNumber = BANK_IDENTIFICATION_CODE.substring(BANK_IDENTIFICATION_CODE.length() - NUM_OF_CHARS_TO_USE);
        String personalAccountNumber = BALANCE_ACCOUNT_OF_AN_INDIVIDUAL + CURRENCY_CODE_OF_THE_RUSSIAN_RUBLE + CONTROL_KEY_PLACEHOLDER + BANKING_DIVISION + (PERSONAL_ACCOUNT_NUMBER_LOWER_BOUND + random.nextInt(PERSONAL_ACCOUNT_NUMBER_UPPER_BOUND));
        String mask = organizationNumber + personalAccountNumber;
        int lowestDigitsSum = 0;
        for (int i = 0; i < WEIGHTS.length(); i++) {
            lowestDigitsSum += (Integer.parseInt(String.valueOf(mask.charAt(i))) * Integer.parseInt(String.valueOf(WEIGHTS.charAt(i)))) % LAST_CHAR_DIVIDER;
        }
        int controlKey = ((lowestDigitsSum % LAST_CHAR_DIVIDER) * DIGIT_SUM_MULTIPLIER) % LAST_CHAR_DIVIDER;
        return BALANCE_ACCOUNT_OF_AN_INDIVIDUAL + CURRENCY_CODE_OF_THE_RUSSIAN_RUBLE + controlKey + BANKING_DIVISION + (PERSONAL_ACCOUNT_NUMBER_LOWER_BOUND + random.nextInt(PERSONAL_ACCOUNT_NUMBER_UPPER_BOUND));
    }

    public String generateCorrespondentAccountNumber() {
        return FIRST_RUSSIAN_CORRESPONDENT_NUMBERS + SECOND_RUSSIAN_CORRESPONDENT_NUMBERS + CURRENCY_CODE_OF_THE_RUSSIAN_RUBLE +
